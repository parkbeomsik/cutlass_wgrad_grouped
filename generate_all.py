...
def main():
    ...
    # Sweep parameters
    tb_shape_0_cand = [8, 16, 32, 64, 128]
    tb_shape_1_cand = [8, 16, 32, 64, 128]
    tb_shape_2_cand = [2, 4, 8 ,16, 32]
    mma_shape_0_cand = [8, 16, 32, 64, 128]
    mma_shape_1_cand = [8, 16, 32, 64, 128]
    mma_shape_2_cand = [2, 4, 8 ,16, 32]
    instruction_shape_0_cand = [1]
    instruction_shape_1_cand = [1]
    instruction_shape_2_cand = [1, 4]
    num_stages_cand = [3, 4, 5]
    ...
    log_f_name = "compile_results.json"
    ...
    for (tb_0, tb_1, tb_2,
        ...
        kernel_name = generate_kernel.get_kernel_name("float", "OpClassSimt", (tb_0, tb_1, tb_2), (mma_0, mma_1, mma_2), (inst_0, inst_1, inst_2), num_stages)           

        try:                                
            if kernel_name in compile_results["results"].keys(): 
                result = compile_results["results"][result]
            
            else:
                result=None
                if (tb_0 < mma_0) or (tb_1 < mma_1) or (tb_2 < mma_2) or \
                    (mma_0 < inst_0) or (mma_1 < inst_1) or (mma_2 < inst_2):
                    result="Failed"
                    raise RuntimeError
                ...
                with open(test_source_file_name, "w") as f:
                    f.write(generate_test_kernel.get_string("float", "OpClassSimt", (tb_0, tb_1, tb_2), (mma_0, mma_1, mma_2), (inst_0, inst_1, inst_2), num_stages))
                ...
                result = "Success"
                with open(f"src/kernel/{kernel_name}.cu", "w") as f:
                    f.write(generate_kernel.get_string("float", "OpClassSimt", (tb_0, tb_1, tb_2), (mma_0, mma_1, mma_2), (inst_0, inst_1, inst_2), num_stages))
    ...
    with open("src/initialize_all.h", "w") as f:
        f.write(generate_initialize_all_h.get_string(success_kernel_names))

    with open("src/initialize_all.cpp", "w") as f:
